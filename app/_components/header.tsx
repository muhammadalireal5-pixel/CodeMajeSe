  ...
  return (
    <div className=' p-4 max-w-7xl flex justify-between items-center w-full ' >
      <div className=' flex gap-2 items-center ' >
        <Image src={'/sun.png'} alt='logo' width={40} height={40} loading="eager" />
        <h2 className=' font-bold text-3xl font-game ' >CMS</h2>
      </div>
        ...
        <NavigationMenuList className="gap-8" >
          ...
          {navItems.map((item) => (
            <NavigationMenuItem key={item.title}>
              <NavigationMenuLink render={<Link href={item.link}>{item.title}</Link>} />
            </NavigationMenuItem>
          ))}
        </NavigationMenuList>
      </NavigationMenu>
